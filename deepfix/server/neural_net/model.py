...
class model:
    ...
    def __init__(self, in_seq_length, out_seq_length, vocabulary_size, rnn_cell="GRU", memory_dim=300, num_layers=4, dropout=0.2, embedding_dim=50, bidirectional=False, trainable=True):
        ...
        self.saver    = tf.train.Saver(tf.global_variables(), max_to_keep=5,write_version=tf.train.SaverDef.V1)
#	self.saver    = tf.train.Saver()
	...
	#test_batch_size = 4
	#test_sequence_max_len = 5
	#test_num_classes = 3

	#test_y_pred = tf.placeholder(tf.float32,shape=(test_batch_size,test_sequence_max_len,test_num_classes))
	#test_y_pred_transposed = tf.transpose(test_y_pred,perm=[1,0,2])
	
	#logits = tf.log(test_y_pred_transposed)
	#test_sequence_length = tf.to_int32(tf.fill([test_batch_size],test_sequence_max_len))
	#test_decoded,test_log_probabilities = tf.nn.ctc_beam_search_decoder(logits,test_sequence_max_len,beam_width=3,merge_repeated=False,top_paths=1)
	...
	print "dec_outputs_batch :",dec_outputs_batch

	#print "test_decoded :",test_decoded
	#print "test_log_probabilities :",test_log_probabilities
	print "Y_hat :",Y_hat
	print "np.array(Y_hat) :",np.array(Y_hat)
	print "np.array(Y_hat).T :",np.array(Y_hat).T
